...
import { IGSMLockLongCall, useGetDTCurrentGSMLockLongCall_MCIQuery } from '../../redux/features/probSocketApiSlice';
...
interface MCIGSMLockLongCallLMapProps { }

const MCIGSMLockLongCallLMap: React.FC<MCIGSMLockLongCallLMapProps> = () => {
    ...
    return (
        <Stack direction={'column'} width={1} height={1} spacing={1}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                <Chip label={<Typography sx={{ fontSize: 9.5 }} variant='caption'>2G Lock Long Call <small>MCI</small></Typography>} color="info" />
            </Box>
            {
...
const GMap: React.FC<GMapProps> = () => {
    const { data: gsmLongCallLockData_MCI = [] } = useGetDTCurrentGSMLockLongCall_MCIQuery()
    const [showTable, setShowTable] = useState<boolean>(false);
    const theme = useTheme();

    const lastLat = +(gsmLongCallLockData_MCI.slice(-1)[0]?.latitude)
    const lastLng = +(gsmLongCallLockData_MCI.slice(-1)[0]?.longitude)
    const lastAlt = +(gsmLongCallLockData_MCI.slice(-1)[0]?.altitude)

    const lastRxQual = +(gsmLongCallLockData_MCI.slice(-1)[0]?.gsmLongCallSamplesMCI[0]?.rxqualsub) || +(gsmLongCallLockData_MCI.slice(-2)[0]?.gsmLongCallSamplesMCI[0]?.rxqualsub)

    return (
        <Stack direction={'column'}>
            <Box sx={{ position: 'relative', display: 'inline-block', p: 0, m: 0, width: 1, height: 1 }}>
                {
                    gsmLongCallLockData_MCI &&
                    <APIProvider apiKey={"AIzaSyBTAu6wiVZVn6sajQl-DM2TkY0oKon2MLk"} >
                        <Map
                            mapId={'bf51a910020fa25a_gsmLongCallLockData_MCI'}
                            style={{ borderRadius: "4px", minHeight: "400px" }}
                            defaultCenter={{ lat: (gsmLongCallLockData_MCI && gsmLongCallLockData_MCI.slice(-1)[0] && +gsmLongCallLockData_MCI.slice(-1)[0].latitude) || 38.026946, lng: (gsmLongCallLockData_MCI && gsmLongCallLockData_MCI.slice(-1)[0] && +gsmLongCallLockData_MCI.slice(-1)[0].longitude) || 46.369456 }}
                            defaultZoom={15}
                        ...
                        >
                            {
                                gsmLongCallLockData_MCI?.map((point, index) =>
                                    <AdvancedMarker
                                        key={index}
                                        position={{ lat: +point.latitude, lng: +point.longitude }}
                                        title={'AdvancedMarker with custom html content.'}
                                    >
                                        {getRxQualColoredDot(point.gsmLongCallSamplesMCI && point.gsmLongCallSamplesMCI[0] && +point.gsmLongCallSamplesMCI[0].rxqualsub)}
                                    </AdvancedMarker>
                                )
                            }
                        </Map>
            ...
            {
                gsmLongCallLockData_MCI.length > 0 ?
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', width: 1 }}>
                        <Typography variant='caption'>
                            {lastLat.toFixed(4)}, {lastLng.toFixed(4)}, {lastAlt.toFixed(0)}
                        </Typography>
                        <Typography variant='caption'>
                            RxQual: {lastRxQual} | samples: {gsmLongCallLockData_MCI.length}
                        </Typography>
                    </Box> :
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', width: 1 }}>
                        <Typography variant='caption'>
                        ...
                        <Typography variant='caption'>
                            RxQual: - | samples: {gsmLongCallLockData_MCI.length}
                        </Typography>
                    </Box>
            }
        </Stack>
...
const MapLibre: React.FC<MapLibreProps> = ({ mapStyle }) => {
        ...
        return gsmLongCallLockData.map(p => (
            {
                ...
                properties: {
                    color: getRxQualColor((p.gsmLongCallSamplesMCI && p.gsmLongCallSamplesMCI.length > 0) ? +p.gsmLongCallSamplesMCI[0]?.rxqualsub : 0),
                    value: (p.gsmLongCallSamplesMCI && p.gsmLongCallSamplesMCI.length > 0) ? +p.gsmLongCallSamplesMCI[0]?.rxqualsub : 0,
                    description: `<div style=color:black;><strong>RxQual</strong>: ${(p.gsmLongCallSamplesMCI && p.gsmLongCallSamplesMCI.length > 0) ? +p.gsmLongCallSamplesMCI[0]?.rxqualsub : 0} dbm</div>`
                }
            }
        ))
            ||
    ...
    const addLayersAndSources = () => {
        if (map.current) {
            ...
            const geojson: GeoJSON.FeatureCollection = {
                ...
                features: getGeojsonFeatures(gsmLongCallLockData_MCI)
            };
            ...
            const lng = gsmLongCallLockData_MCI.slice(-1)[0] && +gsmLongCallLockData_MCI.slice(-1)[0].longitude
            const lat = gsmLongCallLockData_MCI.slice(-1)[0] && +gsmLongCallLockData_MCI.slice(-1)[0].latitude
            if (lng && lat) {
    ...
    useEffect(() => {
        if (map.current) {
            ...
            if (src && gsmLongCallLockData_MCI.length > 0) {
                src.setData(geojson)
                const lng = +gsmLongCallLockData_MCI.slice(-1)[0].longitude
                const lat = +gsmLongCallLockData_MCI.slice(-1)[0].latitude
                if (lat && lng)
                    map.current.panTo([+gsmLongCallLockData_MCI.slice(-1)[0].longitude, +gsmLongCallLockData_MCI.slice(-1)[0].latitude])
            }
        }
        // eslint-disable-next-line
    }, [gsmLongCallLockData_MCI])
...
const TableGuide: React.FC<TableGuideProps> = () => {
    ...
    const samples = gsmLongCallLockData_MCI.filter(item => item.latitude).map(sm => {
        const rxqualsub = sm.gsmLongCallSamplesMCI && sm.gsmLongCallSamplesMCI[0] && sm.gsmLongCallSamplesMCI[0].rxqualsub
    ...
    for (let i = 1; i < gsmLongCallLockData_MCI.filter(item => item.latitude).length; i++) {
        const sm = gsmLongCallLockData_MCI[i];
        const rxqualsub = sm.gsmLongCallSamplesMCI && sm.gsmLongCallSamplesMCI[0] && sm.gsmLongCallSamplesMCI[0].rxqualsub
        const rxqualsubColor = getRxQualColorEnum(rxqualsub)
        if (i > 0) {
            const currentLat = +sm.latitude
            const currentLng = +sm.longitude
            const prevLat = +gsmLongCallLockData_MCI[i - 1].latitude
            const prevLng = +gsmLongCallLockData_MCI[i - 1].longitude
            ...
            <Box component={'table'} sx={{ zIndex: 1, borderCollapse: 'collapse', fontSize: 10, border: 1, maxWidth: 120, borderRadius: '4px', color: theme.palette.common.black }} >
                <Box component={'tbody'}>
                    <Box component={'tr'} borderBottom={1}>
                        <Box component={'th'} color={theme.palette.info.main} colSpan={2}><Typography sx={{ fontSize: 9 }} noWrap>RxQual <small>MCI</small></Typography></Box>
...
export default MCIGSMLockLongCallLMap
