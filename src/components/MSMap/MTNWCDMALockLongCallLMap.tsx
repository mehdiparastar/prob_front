...
import { IWCDMALockLongCall, useGetDTCurrentWCDMALockLongCall_MTNQuery } from '../../redux/features/probSocketApiSlice';
...
interface MTNWCDMALockLongCallLMapProps { }

const MTNWCDMALockLongCallLMap: React.FC<MTNWCDMALockLongCallLMapProps> = () => {
    ...
    return (
        <Stack direction={'column'} width={1} height={1} spacing={1}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                <Chip label={<Typography sx={{ fontSize: 9.5 }} variant='caption'>3G Lock Long Call <small>MTN</small></Typography>} color="info" />
            </Box>
            {
...
const MapLibre: React.FC<MapLibreProps> = ({ mapStyle }) => {
        ...
        return wcdmaLongCallLockData.map(p => (
            {
                ...
                properties: {
                    color: getECIOColor((p.wcdmaLongCallSamplesMTN && p.wcdmaLongCallSamplesMTN.length > 0) ? +p.wcdmaLongCallSamplesMTN[0]?.ecio : 0),
                    value: (p.wcdmaLongCallSamplesMTN && p.wcdmaLongCallSamplesMTN.length > 0) ? +p.wcdmaLongCallSamplesMTN[0]?.ecio : 0,
                    description: `<div style=color:black;><strong>ECIO</strong>: ${(p.wcdmaLongCallSamplesMTN && p.wcdmaLongCallSamplesMTN.length > 0) ? +p.wcdmaLongCallSamplesMTN[0]?.ecio : 0} dbm</div>`
                }
            }
        ))
            ||
    ...
    const addLayersAndSources = () => {
        if (map.current) {
            ...
            const geojson: GeoJSON.FeatureCollection = {
                ...
                features: getGeojsonFeatures(wcdmaLongCallLockData_MTN)
            };
            ...
            const lng = wcdmaLongCallLockData_MTN.slice(-1)[0] && +wcdmaLongCallLockData_MTN.slice(-1)[0].longitude
            const lat = wcdmaLongCallLockData_MTN.slice(-1)[0] && +wcdmaLongCallLockData_MTN.slice(-1)[0].latitude
            if (lng && lat) {
    ...
    useEffect(() => {
        if (map.current) {
            ...
            if (src && wcdmaLongCallLockData_MTN.length > 0) {
                src.setData(geojson)
                const lng = +wcdmaLongCallLockData_MTN.slice(-1)[0].longitude
                const lat = +wcdmaLongCallLockData_MTN.slice(-1)[0].latitude
                if (lat && lng)
                    map.current.panTo([+wcdmaLongCallLockData_MTN.slice(-1)[0].longitude, +wcdmaLongCallLockData_MTN.slice(-1)[0].latitude])
            }
        }
        // eslint-disable-next-line
    }, [wcdmaLongCallLockData_MTN])
...
const TableGuide: React.FC<TableGuideProps> = () => {
    ...
    const samples = wcdmaLongCallLockData_MTN.filter(item => item.latitude).map(sm => {
        const ecio = sm.wcdmaLongCallSamplesMTN && sm.wcdmaLongCallSamplesMTN[0] && sm.wcdmaLongCallSamplesMTN[0].ecio
    ...
    for (let i = 1; i < wcdmaLongCallLockData_MTN.filter(item => item.latitude).length; i++) {
        const sm = wcdmaLongCallLockData_MTN[i];
        const ecio = sm.wcdmaLongCallSamplesMTN && sm.wcdmaLongCallSamplesMTN[0] && sm.wcdmaLongCallSamplesMTN[0].ecio
        const ecioColor = getECIOColorEnum(ecio)
        if (i > 0) {
            const currentLat = +sm.latitude
            const currentLng = +sm.longitude
            const prevLat = +wcdmaLongCallLockData_MTN[i - 1].latitude
            const prevLng = +wcdmaLongCallLockData_MTN[i - 1].longitude
            ...
            <Box component={'table'} sx={{ zIndex: 1, borderCollapse: 'collapse', fontSize: 10, border: 1, maxWidth: 120, borderRadius: '4px', color: theme.palette.common.black }} >
                <Box component={'tbody'}>
                    <Box component={'tr'} borderBottom={1}>
                        <Box component={'th'} color={theme.palette.info.main} colSpan={2}><Typography sx={{ fontSize: 9 }} noWrap>ECIO <small>MTN</small></Typography></Box>
...
export default MTNWCDMALockLongCallLMap
