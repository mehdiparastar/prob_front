...
interface MCIGSMLockIdleLMapProps { }

const MCIGSMLockIdleLMap: React.FC<MCIGSMLockIdleLMapProps> = () => {
    ...
    return (
        <Stack direction={'column'} width={1} height={1} spacing={1}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                <Chip label={<Typography variant='caption'>2G Lock IDLE <small>MCI</small></Typography>} color="info" />
            </Box>
            {
            ...
            <Box sx={{ position: 'relative', display: 'inline-block', p: 0, m: 0, width: 1, height: 1 }}>
                {
                    ...
                    <APIProvider apiKey={"AIzaSyBTAu6wiVZVn6sajQl-DM2TkY0oKon2MLk"} >
                        <Map
                            mapId={'bf51a910020fa25a_gsmIdleLockData_MCI'}
...
const MapLibre: React.FC<MapLibreProps> = ({ mapStyle }) => {
    ...
    const addLayersAndSources = () => {
        if (map.current) {
            ...
            const lng = gsmIdleLockData_MCI.slice(-1)[0] && +gsmIdleLockData_MCI.slice(-1)[0].longitude
            const lat = gsmIdleLockData_MCI.slice(-1)[0] && +gsmIdleLockData_MCI.slice(-1)[0].latitude
            if (lng && lat) {
            ...
            <Box component={'table'} sx={{ zIndex: 1, borderCollapse: 'collapse', fontSize: 10, border: 1, maxWidth: 120, borderRadius: '4px', color: theme.palette.common.black }} >
                <Box component={'tbody'}>
                    <Box component={'tr'} borderBottom={1}>
                        <Box component={'th'} color={theme.palette.info.main} colSpan={2}>RxLevel <small>MCI</small></Box>
...
export default MCIGSMLockIdleLMap
