...
import { IWCDMALockIdle, useGetDTCurrentWCDMALockIdle_MTNQuery } from '../../redux/features/probSocketApiSlice';
...
interface MTNWCDMALockIdleLMapProps { }

const MTNWCDMALockIdleLMap: React.FC<MTNWCDMALockIdleLMapProps> = () => {
    ...
    return (
        <Stack direction={'column'} width={1} height={1} spacing={1}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                <Chip label={<Typography variant='caption'>3G Lock IDLE <small>MTN</small></Typography>} color="info" />
            </Box>
            {
...
const GMap: React.FC<GMapProps> = () => {
    const { data: wcdmaIdleLockData_MTN = [] } = useGetDTCurrentWCDMALockIdle_MTNQuery()
    const [showTable, setShowTable] = useState<boolean>(false);
    const theme = useTheme();

    const lastLat = +(wcdmaIdleLockData_MTN.slice(-1)[0]?.latitude)
    const lastLng = +(wcdmaIdleLockData_MTN.slice(-1)[0]?.longitude)
    const lastAlt = +(wcdmaIdleLockData_MTN.slice(-1)[0]?.altitude)

    const lastRSCP = +(wcdmaIdleLockData_MTN.slice(-1)[0]?.wcdmaIdleSamplesMTN[0]?.rscp) || +(wcdmaIdleLockData_MTN.slice(-2)[0]?.wcdmaIdleSamplesMTN[0]?.rscp)

    return (
        <Stack direction={'column'}>
            <Box sx={{ position: 'relative', display: 'inline-block', p: 0, m: 0, width: 1, height: 1 }}>
                {
                    wcdmaIdleLockData_MTN &&
                    <APIProvider apiKey={"AIzaSyBTAu6wiVZVn6sajQl-DM2TkY0oKon2MLk"} >
                        <Map
                            mapId={'bf51a910020fa25a_wcdmaIdleLockData_MTN'}
                            style={{ borderRadius: "4px", minHeight: "400px" }}
                            defaultCenter={{ lat: (wcdmaIdleLockData_MTN && wcdmaIdleLockData_MTN.slice(-1)[0] && +wcdmaIdleLockData_MTN.slice(-1)[0].latitude) || 38.026946, lng: (wcdmaIdleLockData_MTN && wcdmaIdleLockData_MTN.slice(-1)[0] && +wcdmaIdleLockData_MTN.slice(-1)[0].longitude) || 46.369456 }}
                            defaultZoom={15}
                        ...
                        >
                            {
                                wcdmaIdleLockData_MTN?.map((point, index) =>
                                    <AdvancedMarker
                                        key={index}
                                        position={{ lat: +point.latitude, lng: +point.longitude }}
                                        title={'AdvancedMarker with custom html content.'}
                                    >
                                        {getRSCPColoredDot(point.wcdmaIdleSamplesMTN && point.wcdmaIdleSamplesMTN[0] && +point.wcdmaIdleSamplesMTN[0].rscp)}
                                    </AdvancedMarker>
                                )
                            }
                        </Map>
            ...
            {
                wcdmaIdleLockData_MTN.length > 0 ?
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', width: 1 }}>
                        <Typography variant='caption'>
                            {lastLat.toFixed(4)}, {lastLng.toFixed(4)}, {lastAlt.toFixed(0)}
                        </Typography>
                        <Typography variant='caption'>
                            RSCP: {lastRSCP} | samples: {wcdmaIdleLockData_MTN.length}
                        </Typography>
                    </Box> :
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', width: 1 }}>
                        <Typography variant='caption'>
                        ...
                        <Typography variant='caption'>
                            RSCP: - | samples: {wcdmaIdleLockData_MTN.length}
                        </Typography>
                    </Box>
            }
        </Stack>
...
const MapLibre: React.FC<MapLibreProps> = ({ mapStyle }) => {
    ...
    const getGeojsonFeatures = (wcdmaIdleLockData: IWCDMALockIdle[]): GeoJSON.Feature[] => {
        return wcdmaIdleLockData.map(p => (
            {
                type: 'Feature',
                geometry: {
                    type: 'Point',
                ...
                properties: {
                    color: getRSCPColor((p.wcdmaIdleSamplesMTN && p.wcdmaIdleSamplesMTN.length > 0) ? +p.wcdmaIdleSamplesMTN[0]?.rscp : 0),
                    value: (p.wcdmaIdleSamplesMTN && p.wcdmaIdleSamplesMTN.length > 0) ? +p.wcdmaIdleSamplesMTN[0]?.rscp : 0,
                    description: `<div style=color:black;><strong>RSCP</strong>: ${(p.wcdmaIdleSamplesMTN && p.wcdmaIdleSamplesMTN.length > 0) ? +p.wcdmaIdleSamplesMTN[0]?.rscp : 0} dbm</div>`
                }
            }
        ))
            ||
    ...
    const addLayersAndSources = () => {
        if (map.current) {
            ...
            const geojson: GeoJSON.FeatureCollection = {
                ...
                features: getGeojsonFeatures(wcdmaIdleLockData_MTN)
            };
            ...
            const lng = wcdmaIdleLockData_MTN.slice(-1)[0] && +wcdmaIdleLockData_MTN.slice(-1)[0].longitude
            const lat = wcdmaIdleLockData_MTN.slice(-1)[0] && +wcdmaIdleLockData_MTN.slice(-1)[0].latitude
            if (lng && lat) {
    ...
    useEffect(() => {
        if (map.current) {
            ...
            if (src && wcdmaIdleLockData_MTN.length > 0) {
                src.setData(geojson)
                const lng = +wcdmaIdleLockData_MTN.slice(-1)[0].longitude
                const lat = +wcdmaIdleLockData_MTN.slice(-1)[0].latitude
                if (lat && lng)
                    map.current.panTo([+wcdmaIdleLockData_MTN.slice(-1)[0].longitude, +wcdmaIdleLockData_MTN.slice(-1)[0].latitude])
            }
        }
        // eslint-disable-next-line
    }, [wcdmaIdleLockData_MTN])

    const getRSCPColor = (rscp: number | string) => {
        const colorEnum = getRSCPColorEnum(rscp)
...
const TableGuide: React.FC<TableGuideProps> = () => {
    ...
    const samples = wcdmaIdleLockData_MTN.filter(item => item.latitude).map(sm => {
        const rscp = sm.wcdmaIdleSamplesMTN && sm.wcdmaIdleSamplesMTN[0] && sm.wcdmaIdleSamplesMTN[0].rscp
        return getRSCPColorEnum(rscp)
    })
    ...
    for (let i = 1; i < wcdmaIdleLockData_MTN.filter(item => item.latitude).length; i++) {
        const sm = wcdmaIdleLockData_MTN[i];
        const rscp = sm.wcdmaIdleSamplesMTN && sm.wcdmaIdleSamplesMTN[0] && sm.wcdmaIdleSamplesMTN[0].rscp
        const rscpColor = getRSCPColorEnum(rscp)
        if (i > 0) {
            const currentLat = +sm.latitude
            const currentLng = +sm.longitude
            const prevLat = +wcdmaIdleLockData_MTN[i - 1].latitude
            const prevLng = +wcdmaIdleLockData_MTN[i - 1].longitude
            ...
            <Box component={'table'} sx={{ zIndex: 1, borderCollapse: 'collapse', fontSize: 10, border: 1, maxWidth: 120, borderRadius: '4px', color: theme.palette.common.black }} >
                <Box component={'tbody'}>
                    <Box component={'tr'} borderBottom={1}>
                        <Box component={'th'} color={theme.palette.info.main} colSpan={2}>RSCP <small>MTN</small></Box>
                    ...
                    <Box component={'tr'} borderBottom={1}>
                        <Box component={'th'}>
                            <Typography noWrap variant='caption'>[-105, +∞)</Typography>
                        </Box>
                    ...
                    <Box component={'tr'} borderBottom={1}>
                        <Box component={'th'}>
                            <Typography noWrap variant='caption'>[-113, -105)</Typography>
                        </Box>
                    ...
                    <Box component={'tr'} borderBottom={1}>
                        <Box component={'th'}>
                            <Typography noWrap variant='caption'>(-∞, -113)</Typography>
                        </Box>
...
export const getRSCPColoredDot = (rscp: number | string) => {
    const colorEnum = getRSCPColorEnum(rscp)
...
export const getRSCPColorEnum = (rscp: number | string) => {
    if (Number.isNaN(+rscp) === false || (typeof (rscp) === 'string' && rscp.trim() !== '')) {
        if (+rscp < -113) {
            return DotColorsEnum.red
        } else if (+rscp >= -105) {
            return DotColorsEnum.green
        } else if (+rscp >= -113 && +rscp < -105) {
...
export default MTNWCDMALockIdleLMap
