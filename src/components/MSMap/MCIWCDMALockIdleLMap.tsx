...
import { IWCDMALockIdle, useGetDTCurrentWCDMALockIdle_MCIQuery } from '../../redux/features/probSocketApiSlice';
...
interface MCIWCDMALockIdleLMapProps { }

const MCIWCDMALockIdleLMap: React.FC<MCIWCDMALockIdleLMapProps> = () => {
    ...
    return (
        <Stack direction={'column'} width={1} height={1} spacing={1}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                <Chip label={<Typography variant='caption'>3G Lock IDLE <small>MCI</small></Typography>} color="info" />
            </Box>
            {
...
const MapLibre: React.FC<MapLibreProps> = ({ mapStyle }) => {
        ...
        return wcdmaIdleLockData.map(p => (
            {
                ...
                properties: {
                    color: getRSCPColor((p.wcdmaIdleSamplesMCI && p.wcdmaIdleSamplesMCI.length > 0) ? +p.wcdmaIdleSamplesMCI[0]?.rscp : 0),
                    value: (p.wcdmaIdleSamplesMCI && p.wcdmaIdleSamplesMCI.length > 0) ? +p.wcdmaIdleSamplesMCI[0]?.rscp : 0,
                    description: `<div style=color:black;><strong>RSCP</strong>: ${(p.wcdmaIdleSamplesMCI && p.wcdmaIdleSamplesMCI.length > 0) ? +p.wcdmaIdleSamplesMCI[0]?.rscp : 0} dbm</div>`
                }
            }
        ))
            ||
    ...
    const addLayersAndSources = () => {
        if (map.current) {
            ...
            const geojson: GeoJSON.FeatureCollection = {
                ...
                features: getGeojsonFeatures(wcdmaIdleLockData_MCI)
            };
            ...
            const lng = wcdmaIdleLockData_MCI.slice(-1)[0] && +wcdmaIdleLockData_MCI.slice(-1)[0].longitude
            const lat = wcdmaIdleLockData_MCI.slice(-1)[0] && +wcdmaIdleLockData_MCI.slice(-1)[0].latitude
            if (lng && lat) {
    ...
    useEffect(() => {
        if (map.current) {
            ...
            if (src && wcdmaIdleLockData_MCI.length > 0) {
                src.setData(geojson)
                const lng = +wcdmaIdleLockData_MCI.slice(-1)[0].longitude
                const lat = +wcdmaIdleLockData_MCI.slice(-1)[0].latitude
                if (lat && lng)
                    map.current.panTo([+wcdmaIdleLockData_MCI.slice(-1)[0].longitude, +wcdmaIdleLockData_MCI.slice(-1)[0].latitude])
            }
        }
        // eslint-disable-next-line
    }, [wcdmaIdleLockData_MCI])
...
const TableGuide: React.FC<TableGuideProps> = () => {
    ...
    const samples = wcdmaIdleLockData_MCI.filter(item => item.latitude).map(sm => {
        const rscp = sm.wcdmaIdleSamplesMCI && sm.wcdmaIdleSamplesMCI[0] && sm.wcdmaIdleSamplesMCI[0].rscp
    ...
    for (let i = 1; i < wcdmaIdleLockData_MCI.filter(item => item.latitude).length; i++) {
        const sm = wcdmaIdleLockData_MCI[i];
        const rscp = sm.wcdmaIdleSamplesMCI && sm.wcdmaIdleSamplesMCI[0] && sm.wcdmaIdleSamplesMCI[0].rscp
        const rscpColor = getRSCPColorEnum(rscp)
        if (i > 0) {
            const currentLat = +sm.latitude
            const currentLng = +sm.longitude
            const prevLat = +wcdmaIdleLockData_MCI[i - 1].latitude
            const prevLng = +wcdmaIdleLockData_MCI[i - 1].longitude
            ...
            <Box component={'table'} sx={{ zIndex: 1, borderCollapse: 'collapse', fontSize: 10, border: 1, maxWidth: 120, borderRadius: '4px', color: theme.palette.common.black }} >
                <Box component={'tbody'}>
                    <Box component={'tr'} borderBottom={1}>
                        <Box component={'th'} color={theme.palette.info.main} colSpan={2}>RSCP <small>MCI</small></Box>
...
export default MCIWCDMALockIdleLMap
