...
import { ILTELockIdle, useGetDTCurrentLTELockIdle_MTNQuery } from '../../redux/features/probSocketApiSlice';
...
interface MTNLTELockIdleLMapProps { }

const MTNLTELockIdleLMap: React.FC<MTNLTELockIdleLMapProps> = () => {
    ...
    return (
        <Stack direction={'column'} width={1} height={1} spacing={1}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                <Chip label={<Typography variant='caption'>4G Lock IDLE <small>MTN</small></Typography>} color="info" />
            </Box>
            {
...
const MapLibre: React.FC<MapLibreProps> = ({ mapStyle }) => {
        ...
        return lteIdleLockData.map(p => (
            {
                ...
                properties: {
                    color: getRSRPColor((p.lteIdleSamplesMTN && p.lteIdleSamplesMTN.length > 0) ? +p.lteIdleSamplesMTN[0]?.rsrp : 0),
                    value: (p.lteIdleSamplesMTN && p.lteIdleSamplesMTN.length > 0) ? +p.lteIdleSamplesMTN[0]?.rsrp : 0,
                    description: `<div style=color:black;><strong>RSRP</strong>: ${(p.lteIdleSamplesMTN && p.lteIdleSamplesMTN.length > 0) ? +p.lteIdleSamplesMTN[0]?.rsrp : 0} dbm</div>`
                }
            }
        ))
            ||
    ...
    const addLayersAndSources = () => {
        if (map.current) {
            ...
            const geojson: GeoJSON.FeatureCollection = {
                ...
                features: getGeojsonFeatures(lteIdleLockData_MTN)
            };
            ...
            const lng = lteIdleLockData_MTN.slice(-1)[0] && +lteIdleLockData_MTN.slice(-1)[0].longitude
            const lat = lteIdleLockData_MTN.slice(-1)[0] && +lteIdleLockData_MTN.slice(-1)[0].latitude
            if (lng && lat) {
    ...
    useEffect(() => {
        if (map.current) {
            ...
            if (src && lteIdleLockData_MTN.length > 0) {
                src.setData(geojson)
                const lng = +lteIdleLockData_MTN.slice(-1)[0].longitude
                const lat = +lteIdleLockData_MTN.slice(-1)[0].latitude
                if (lat && lng)
                    map.current.panTo([+lteIdleLockData_MTN.slice(-1)[0].longitude, +lteIdleLockData_MTN.slice(-1)[0].latitude])
            }
        }
        // eslint-disable-next-line
    }, [lteIdleLockData_MTN])
...
const TableGuide: React.FC<TableGuideProps> = () => {
    ...
    const samples = lteIdleLockData_MTN.filter(item => item.latitude).map(sm => {
        const rsrp = sm.lteIdleSamplesMTN && sm.lteIdleSamplesMTN[0] && sm.lteIdleSamplesMTN[0].rsrp
    ...
    for (let i = 1; i < lteIdleLockData_MTN.filter(item => item.latitude).length; i++) {
        const sm = lteIdleLockData_MTN[i];
        const rsrp = sm.lteIdleSamplesMTN && sm.lteIdleSamplesMTN[0] && sm.lteIdleSamplesMTN[0].rsrp
        const rsrpColor = getRSRPColorEnum(rsrp)
        if (i > 0) {
            const currentLat = +sm.latitude
            const currentLng = +sm.longitude
            const prevLat = +lteIdleLockData_MTN[i - 1].latitude
            const prevLng = +lteIdleLockData_MTN[i - 1].longitude
            ...
            <Box component={'table'} sx={{ zIndex: 1, borderCollapse: 'collapse', fontSize: 10, border: 1, maxWidth: 120, borderRadius: '4px', color: theme.palette.common.black }} >
                <Box component={'tbody'}>
                    <Box component={'tr'} borderBottom={1}>
                        <Box component={'th'} color={theme.palette.info.main} colSpan={2}>RSRP <small>MTN</small></Box>
...
export default MTNLTELockIdleLMap
