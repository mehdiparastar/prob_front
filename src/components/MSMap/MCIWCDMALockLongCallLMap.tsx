...
import { IWCDMALockLongCall, useGetDTCurrentWCDMALockLongCall_MCIQuery } from '../../redux/features/probSocketApiSlice';
...
interface MCIWCDMALockLongCallLMapProps { }

const MCIWCDMALockLongCallLMap: React.FC<MCIWCDMALockLongCallLMapProps> = () => {
    ...
    return (
        <Stack direction={'column'} width={1} height={1} spacing={1}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                <Chip label={<Typography sx={{ fontSize: 9.5 }} variant='caption'>3G Lock Long Call <small>MCI</small></Typography>} color="info" />
            </Box>
            {
...
const GMap: React.FC<GMapProps> = () => {
    const { data: wcdmaLongCallLockData_MCI = [] } = useGetDTCurrentWCDMALockLongCall_MCIQuery()
    const [showTable, setShowTable] = useState<boolean>(false);
    const theme = useTheme();

    const lastLat = +(wcdmaLongCallLockData_MCI.slice(-1)[0]?.latitude)
    const lastLng = +(wcdmaLongCallLockData_MCI.slice(-1)[0]?.longitude)
    const lastAlt = +(wcdmaLongCallLockData_MCI.slice(-1)[0]?.altitude)

    const lastECIO = +(wcdmaLongCallLockData_MCI.slice(-1)[0]?.wcdmaLongCallSamplesMCI[0]?.ecio) || +(wcdmaLongCallLockData_MCI.slice(-2)[0]?.wcdmaLongCallSamplesMCI[0]?.ecio)

    return (
        <Stack direction={'column'}>
            <Box sx={{ position: 'relative', display: 'inline-block', p: 0, m: 0, width: 1, height: 1 }}>
                {
                    wcdmaLongCallLockData_MCI &&
                    <APIProvider apiKey={"AIzaSyBTAu6wiVZVn6sajQl-DM2TkY0oKon2MLk"} >
                        <Map
                            mapId={'bf51a910020fa25a_wcdmaLongCallLockData_MCI'}
                            style={{ borderRadius: "4px", minHeight: "400px" }}
                            defaultCenter={{ lat: (wcdmaLongCallLockData_MCI && wcdmaLongCallLockData_MCI.slice(-1)[0] && +wcdmaLongCallLockData_MCI.slice(-1)[0].latitude) || 38.026946, lng: (wcdmaLongCallLockData_MCI && wcdmaLongCallLockData_MCI.slice(-1)[0] && +wcdmaLongCallLockData_MCI.slice(-1)[0].longitude) || 46.369456 }}
                            defaultZoom={15}
                        ...
                        >
                            {
                                wcdmaLongCallLockData_MCI?.map((point, index) =>
                                    <AdvancedMarker
                                        key={index}
                                        position={{ lat: +point.latitude, lng: +point.longitude }}
                                        title={'AdvancedMarker with custom html content.'}
                                    >
                                        {getECIOColoredDot(point.wcdmaLongCallSamplesMCI && point.wcdmaLongCallSamplesMCI[0] && +point.wcdmaLongCallSamplesMCI[0].ecio)}
                                    </AdvancedMarker>
                                )
                            }
                        </Map>
            ...
            {
                wcdmaLongCallLockData_MCI.length > 0 ?
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', width: 1 }}>
                        <Typography variant='caption'>
                            {lastLat.toFixed(4)}, {lastLng.toFixed(4)}, {lastAlt.toFixed(0)}
                        </Typography>
                        <Typography variant='caption'>
                            ECIO: {lastECIO} | samples: {wcdmaLongCallLockData_MCI.length}
                        </Typography>
                    </Box> :
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', width: 1 }}>
                        <Typography variant='caption'>
                        ...
                        <Typography variant='caption'>
                            ECIO: - | samples: {wcdmaLongCallLockData_MCI.length}
                        </Typography>
                    </Box>
            }
        </Stack>
...
const MapLibre: React.FC<MapLibreProps> = ({ mapStyle }) => {
    ...
    const getGeojsonFeatures = (wcdmaLongCallLockData: IWCDMALockLongCall[]): GeoJSON.Feature[] => {
        return wcdmaLongCallLockData.map(p => (
            {
                type: 'Feature',
                geometry: {
                    type: 'Point',
                ...
                properties: {
                    color: getECIOColor((p.wcdmaLongCallSamplesMCI && p.wcdmaLongCallSamplesMCI.length > 0) ? +p.wcdmaLongCallSamplesMCI[0]?.ecio : 0),
                    value: (p.wcdmaLongCallSamplesMCI && p.wcdmaLongCallSamplesMCI.length > 0) ? +p.wcdmaLongCallSamplesMCI[0]?.ecio : 0,
                    description: `<div style=color:black;><strong>ECIO</strong>: ${(p.wcdmaLongCallSamplesMCI && p.wcdmaLongCallSamplesMCI.length > 0) ? +p.wcdmaLongCallSamplesMCI[0]?.ecio : 0} dbm</div>`
                }
            }
        ))
            ||
    ...
    const addLayersAndSources = () => {
        if (map.current) {
            ...
            const geojson: GeoJSON.FeatureCollection = {
                ...
                features: getGeojsonFeatures(wcdmaLongCallLockData_MCI)
            };
    ...
    useEffect(() => {
        if (map.current) {
            ...
            if (src && wcdmaLongCallLockData_MCI.length > 0) {
                src.setData(geojson)
                const lng = +wcdmaLongCallLockData_MCI.slice(-1)[0].longitude
                const lat = +wcdmaLongCallLockData_MCI.slice(-1)[0].latitude
                if (lat && lng)
    ...
    const getECIOColor = (ecio: number | string) => {
        const colorEnum = getECIOColorEnum(ecio)
...
const TableGuide: React.FC<TableGuideProps> = () => {
    ...
    const samples = wcdmaLongCallLockData_MCI.filter(item => item.latitude).map(sm => {
        const ecio = sm.wcdmaLongCallSamplesMCI && sm.wcdmaLongCallSamplesMCI[0] && sm.wcdmaLongCallSamplesMCI[0].ecio
        return getECIOColorEnum(ecio)
    })
    ...
    for (let i = 1; i < wcdmaLongCallLockData_MCI.filter(item => item.latitude).length; i++) {
        const sm = wcdmaLongCallLockData_MCI[i];
        const ecio = sm.wcdmaLongCallSamplesMCI && sm.wcdmaLongCallSamplesMCI[0] && sm.wcdmaLongCallSamplesMCI[0].ecio
        const ecioColor = getECIOColorEnum(ecio)
        if (i > 0) {
            const currentLat = +sm.latitude
            const currentLng = +sm.longitude
            const prevLat = +wcdmaLongCallLockData_MCI[i - 1].latitude
            const prevLng = +wcdmaLongCallLockData_MCI[i - 1].longitude
            ...
            <Box component={'table'} sx={{ zIndex: 1, borderCollapse: 'collapse', fontSize: 10, border: 1, maxWidth: 120, borderRadius: '4px', color: theme.palette.common.black }} >
                <Box component={'tbody'}>
                    <Box component={'tr'} borderBottom={1}>
                        <Box component={'th'} color={theme.palette.info.main} colSpan={2}><Typography sx={{ fontSize: 9 }} noWrap>ECIO <small>MCI</small></Typography></Box>
                    ...
                    <Box component={'tr'} borderBottom={1}>
                        <Box component={'th'}>
                            <Typography noWrap variant='caption'>[-16, +∞)</Typography>
                        </Box>
                    ...
                    <Box component={'tr'} borderBottom={1}>
                        <Box component={'th'}>
                            <Typography noWrap variant='caption'>[-18, -16)</Typography>
                        </Box>
                    ...
                    <Box component={'tr'} borderBottom={1}>
                        <Box component={'th'}>
                            <Typography noWrap variant='caption'>(-∞, -18)</Typography>
                        </Box>
...
export const getECIOColoredDot = (ecio: number | string) => {
    const colorEnum = getECIOColorEnum(ecio)
...
export const getECIOColorEnum = (ecio: number | string) => {
    if (Number.isNaN(+ecio) === false || (typeof (ecio) === 'string' && ecio.trim() !== '')) {
        if (+ecio < -18) {
            return DotColorsEnum.red
        } else if (+ecio >= -16) {
            return DotColorsEnum.green
        } else if (+ecio >= -18 && +ecio < -16) {
...
export default MCIWCDMALockLongCallLMap
